...
if(isset($_POST['form1'])) {
	    ...
	    if(empty($_POST['email'])) {
	    	...
	    	if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
				...
				$statement->execute(array($_SESSION['vendor']['id']));
				$result = $statement->fetchAll(PDO::FETCH_ASSOC);
				foreach($result as $row) {
					$current_email = $row['email'];
				}

		    	$statement = $pdo->prepare("SELECT * FROM tbl_vendor WHERE email=? and email!=?");
	    ...
	    if($valid == 1) {
			...
			$_SESSION['vendor']['name'] = $_POST['name'];
	    	$_SESSION['vendor']['email'] = $_POST['email'];

			// updating the database
			$statement = $pdo->prepare("UPDATE tbl_vendor SET name=?, email=?, phone=? WHERE id=?");
			$statement->execute(array($_POST['full_name'],$_POST['email'],$_POST['phone'],$_SESSION['vendor']['id']));
...
if(isset($_POST['form2'])) {
    ...
    if($valid == 1) {
    	...
    	if($_SESSION['vendor']['photo']!='') {
    		unlink('../assets/uploads/'.$_SESSION['vendor']['photo']);	
    	}

    	// updating the data
    	$final_name = 'user-'.$_SESSION['vendor']['id'].'.'.$ext;
        move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );
        $_SESSION['vendor']['photo'] = $final_name;

        // updating the database
		$statement = $pdo->prepare("UPDATE tbl_vendor SET photo=? WHERE id=?");
		$statement->execute(array($final_name,$_SESSION['vendor']['id']));
...
if(isset($_POST['form3'])) {
    ...
    if($valid == 1) {
    	...
    	$_SESSION['vendor']['password'] = $_POST['password'];

    	// updating the database
		$statement = $pdo->prepare("UPDATE tbl_vendor SET password=? WHERE id=?");
		$statement->execute(array($_POST['password'],$_SESSION['vendor']['id']));
